      ...
      <tbody>
        ...
        >
          ...
          <td style="width: 27%">
            {{
              item.level == "1"
                ? "در انتظار تایید مسئول مستقیم"
                : item.level == "2"
                ? "در انتظار تایید مسئول حراست"
                : item.level == "3"
                ? "در انتظار تایید مدیر نیروگاه"
                : item.level == "-1"
                ? "عدم تایید مسئول مستقیم"
                : item.level == "-2"
                ? "عدم تایید مسئول حراست"
                : item.level == "-3"
                ? "عدم تایید مدیر نیروگاه"
                : ""
            }}
          </td>
        </tr>
...
export default {
  ...
  methods: {
    getData(event) {
      this.loading = true;
      axios
        .get("http://172.28.232.27:8000/vue-in-process/" + event)
        .then((response) => {
